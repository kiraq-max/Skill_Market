...
public class UtenteDAO {
    ...
    public synchronized void doSave(UtenteBean utente) throws SQLException {
        // La query SQL per l'inserimento. L'ID è solitamente AUTO_INCREMENT nel DB.
        String query = "INSERT INTO Utente (nome, cognome, email, passwordHash, ruolo) VALUES (?, ?, ?, ?, ?)";

        // Usiamo il costrutto try-with-resources per gestire in sicurezza la chiusura delle connessioni
        try (Connection con = ConPool.getConnection(); 
             PreparedStatement ps = con.prepareStatement(query)) {

            // Sostituiamo i placeholder '?' con i valori reali presi dal tuo Bean
    ...
    public synchronized UtenteBean doRetrieveByEmailAndPassword(String email, String passwordHash) throws SQLException {
        ...
        try (Connection con = ConPool.getConnection(); 
            ...
            try (ResultSet rs = ps.executeQuery()) {
                // Se la query restituisce un risultato, costruiamo il tuo UtenteBean
                if (rs.next()) {
                    utente = new UtenteBean();
                    // Usiamo i setter che hai definito per popolare l'oggetto
